...
面向大型代码库的渐进式代码知识库生成器。KBLens 使用 tree-sitter 从 C++、C#、Python、TypeScript 和 JavaScript 源文件中提取 AST 骨架，将其打包为 LLM 友好的批次，生成分层 Markdown 摘要——让 AI 编程助手在不阅读每个文件的情况下获取代码库的结构化上下文。
...
- **基于 AST 的提取** — 使用 tree-sitter 从 C++、C#、Python、TypeScript 和 JavaScript 源文件中提取 class/struct/enum/function 签名。无猜测、无幻觉。
...
2. **AST 提取** — 用 tree-sitter 解析源文件，提取 class/struct/enum/function 骨架和依赖指令
...
- **已删组件** 清理其 `.md` 文件和元数据
- **失败组件** （上次超时/出错）自动重试
- **跳过的组件**（AST token < 50）记录在元数据中，避免重复扫描
...
- **C#**（`.cs`）— 类、结构体、接口、record、枚举、委托、泛型约束、特性标注、XML 文档注释、属性、事件；仅保留 public/protected/internal 成员
